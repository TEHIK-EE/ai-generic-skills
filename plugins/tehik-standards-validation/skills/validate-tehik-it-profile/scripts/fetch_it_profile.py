#!/usr/bin/env python3
"""
Fetch the TEHIK IT Profile document, with cache + bundled-snapshot fallback.

Resolution order:
  1. If --refresh OR cache is missing/older than --max-age-hours: try live download.
  2. Otherwise use the cached copy at ~/.cache/tehik-it-profile/it-profile.md.
  3. If both fail, fall back to the snapshot bundled at references/it-profile-source.md.
...
orchestrator can record provenance in the audit report:
  ...
  {
    "path": "/abs/path/to/it-profile.md",
...
UPSTREAM_URL = (
    "https://raw.githubusercontent.com/TEHIK-EE/infotehnoloogiline-profiil/"
    "refs/heads/main/IT-profiil.en.md"
)
DEFAULT_CACHE_DIR = pathlib.Path(
    os.environ.get("TEHIK_IT_PROFILE_CACHE_DIR")
    or pathlib.Path.home() / ".cache" / "tehik-it-profile"
)
SCRIPT_DIR = pathlib.Path(__file__).resolve().parent
BUNDLED_SNAPSHOT = SCRIPT_DIR.parent / "references" / "it-profile-source.md"
...
def main() -> int:
    ...
    cache_file = args.cache_dir / "it-profile.md"
    ...
    if chosen is None:
        emit({
            "error": "no IT profile source available (live download failed, no cache, no bundled snapshot)",
            "url": args.url,
        })
        return 2
